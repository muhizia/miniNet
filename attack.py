...
IFACE = "s1-eth1"   # Or your default interface
DNS_SERVER_IP = "10.2.1.1"  # Your local IP
...
def dns_responder(local_ip, str):
    ...
    def get_response(pkt, IP):
        if (
            ...
            if "foo.local" in str(pkt["DNS Question Record"].qname):
                spf_resp = IP(dst=pkt[IP].src)/UDP(dport=pkt[UDP].sport, sport=53)/DNS(id=pkt[DNS].id,ancount=1,an=DNSRR(rrname=pkt[DNSQR].qname, rdata=local_ip)/DNSRR(rrname="foo.local",rdata=local_ip))
                send(spf_resp, verbose=0, iface=IFACE)
                return "Spoofed DNS Response Sent: " + pkt[IP].src
